...
describe('useNavigation Hook', () => {
  describe('État Initial', () => {
    ...
    it('devrait marquer la page initiale comme visitée', () => {
      const { result } = renderHook(() => useNavigation('team'))
      
      expect(result.current.hasVisited('team')).toBe(true)
  ...
  describe('Navigation', () => {
    ...
    it('devrait construire un historique complet', () => {
      ...
      act(() => {
        result.current.navigate('map')
        result.current.navigate('team')
        result.current.navigate('person')
      })
      
      expect(result.current.history).toEqual(['event', 'map', 'team', 'person'])
    })
  })
  ...
  describe('Reset', () => {
    ...
    it('devrait réinitialiser les pages visitées', () => {
      ...
      expect(result.current.hasVisited('team')).toBe(false)
    })
  })
