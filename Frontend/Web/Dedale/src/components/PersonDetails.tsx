...
export default function PersonDetails({ person, activeEventId, onClose, onDelete, onUpdate, onTeamClick }: PersonDetailsProps) {
    ...
    useEffect(() => {
        setLoading(true);
        if (activeEventId) {
            invoke<Team[]>("fetch_person_teams", { personId: person.id, eventId: activeEventId }).then(setTeams).catch(console.error);
        }
        else {
    ...
    const startAddingTeam = async () => {
        ...
        try {
            let allTeams;
            if (activeEventId) {
                allTeams = await invoke<Team[]>("fetch_teams", { eventId: activeEventId });
            }
            else {
    ...
    const confirmAddTeam = async () => {
        ...
        try {
            await invoke("add_member", { teamId: selectedTeamId, personId: person.id });
        ...
        <div className="bg-white w-full max-w-sm h-[500px] flex flex-col rounded-xl shadow-2xl overflow-hidden relative">
            ...
            <div className="bg-linear-to-r from-blue-50 to-indigo-50 p-6 text-center border-b border-blue-100 relative shrink-0">
                ...
                {isEditing ? (
                    ...
                    <h2 className="text-xl font-bold text-gray-800 text-transform: capitalize">{person.firstname} {person.lastname}</h2>
                )}
            ...
            <div className="flex border-t border-gray-200 border-b shrink-0">
                <button onClick={() => isEditing ? "" : setActiveTab('infos')} className={`flex-1 py-2 text-xs font-medium cursor-pointer ${activeTab === 'infos' ? 'bg-white text-blue-600 border-b-2 border-blue-600' : 'text-gray-500 hover:bg-gray-100'}`}>Informations</button>
                <button onClick={() => isEditing ? "" : setActiveTab('teams')} className={`flex-1 py-2 text-xs font-medium cursor-pointer ${activeTab === 'teams' ? 'bg-white text-blue-600 border-b-2 border-blue-600' : 'text-gray-500 hover:bg-gray-100'}`}>Équipes ({teams.length})</button>
            </div>
                        ...
                        activeTab === 'teams' && (
                            <div className="space-y-2">
                                {teams.map(team => (
                                    ...
                                    >
                                        <span className="text-sm font-medium text-gray-700 text-transform: capitalize">{team.name}</span>
                                ...
                                {!isEditing && (isAddingTeam ? (
                                    <div className="mt-4 p-3 bg-blue-50 rounded-lg border border-blue-100 animate-in fade-in slide-in-from-top-2">
                                        <div className="flex gap-2">
                                            <select value={selectedTeamId} onChange={(e) => setSelectedTeamId(e.target.value)} className="flex-1 text-transform: capitalize text-sm border border-blue-200 rounded px-2 py-1 outline-none cursor-pointer">
                                                <option value="">Choisir...</option>
                                                {availableTeams.map(t => <option key={t.id} value={t.id}>{t.name}</option>)}
                                            </select>
                                            <button onClick={() => confirmAddTeam()} disabled={!selectedTeamId} className="bg-blue-600 text-white px-3 py-1 rounded text-sm hover:bg-blue-700 cursor-pointer">OK</button>
                                            <button onClick={() => setIsAddingTeam(false)} className="text-gray-500 px-2 cursor-pointer">✕</button>
                                        </div>
                                    </div>
                                ) : (
                                    <button onClick={startAddingTeam} className="w-full py-2 mt-4 border border-dashed border-gray-300 rounded-lg text-gray-500 text-xs hover:border-blue-400 hover:text-blue-600 hover:bg-blue-50 transition-all flex items-center justify-center gap-1 cursor-pointer">
            ...
            <div className="bg-gray-50 p-4 flex justify-between items-center border-t border-gray-100 mt-auto shrink-0">
                {isEditing ? (
                    <div className="flex gap-2 w-full">
                        <button onClick={() => { setIsEditing(false); setEditData({ ...person }); }} className="flex-1 px-3 py-2 bg-white border border-gray-300 rounded text-sm text-gray-700 cursor-pointer">Annuler</button>
                        <button onClick={handleSave} disabled={isSaving} className="flex-1 px-3 py-2 bg-green-600 text-white rounded text-sm hover:bg-green-700 cursor-pointer">{isSaving ? "..." : "Enregistrer"}</button>
                    </div>
                ) : (
                    showConfirm ? (
                        ...
                        <>
                            <button onClick={() => setShowConfirm(true)} className="text-red-500 hover:bg-red-50 p-2 rounded transition-colors cursor-pointer"><svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16" /></svg></button>
                            <button onClick={onClose} className="px-4 py-2 border border-gray-300 bg-white rounded text-xs font-medium text-gray-700 hover:bg-gray-50 cursor-pointer">Fermer</button>
                        </>
                    )
                )}
            </div>
