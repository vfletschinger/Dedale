...
import { faTools, faCheck, faTimes, faCaretDown } from "@fortawesome/free-solid-svg-icons";
    ...
    <div ref={dropdownRef} className={`relative ${isHeader ? "" : "bg-white rounded-lg shadow-lg border border-gray-200 overflow-hidden max-w-xs"}`}>
      ...
      >
        <span className="flex items-center gap-2">
          ...
          {noneSelected && (
            <span className={`text-xs px-1.5 py-0.5 rounded-full ${isHeader ? "bg-red-500 text-white" : "bg-red-100 text-red-700"
              }`}>
              Aucun
            </span>
          )}
